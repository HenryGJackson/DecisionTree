...
# ----------------------------------------------------------------------------------------------------------------------
    def create_network(self, win_df, lose_df, step_size):
        ...
        # print (self.seed.get_weight())k: Could not create seed. Exiting...")
    ...
    # splits the input node between pass and fail
    ...
    # Find the variable and function combination for this node that maximises the number of correctly classified events
    def prepare_node(self, win_df, lose_df, node, step_size=10):
        if node is None:
            ...
            for variable in variables:
                ...
                if ((win_df[variable].dtype != np.int64) & (win_df[variable].dtype != np.float64)) | (
                self.check_variable_use(variable)):
    ...
    # Check if one of the daughter nodes to the node passed in should be set to output
    # Create the nodes for output and return the number of nodes created
    ...
    # Assess whether the ratio of win entries to lose entries is extreme
    ...
    # crete a node with the properties of an output node
